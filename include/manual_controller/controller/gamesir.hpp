...
class GamesirController : public Controller
{
public:
  GamesirController() : Controller() {}
  GamesirController(double threshold) : Controller(threshold) {}
  double get_axis(Axis axis) const
  {
    ...
    // if (axis_map == Axis_Map::LEFT_Y || axis_map == Axis_Map::RIGHT_Y)
    //   return -1*get_axis_f(static_cast<size_t>(axis_map));
    return -1*get_axis_f(static_cast<size_t>(axis_map));
  }
...
private:
  ...
  std::unordered_map<Key, Key_Map> key_map_ = {
    ...
    {Key::UP, Key_Map::UP},
    {Key::DOWN, Key_Map::DOWN},
    {Key::LEFT, Key_Map::LEFT},
    {Key::RIGHT, Key_Map::RIGHT},
